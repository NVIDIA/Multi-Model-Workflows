...
"""Instantiate the docker container for development."""
...
ROOT_DIR = os.getenv("REPO_TOP", os.getcwd())
...
def instantiate_dev_docker(gpus, mount_file,
    ...
    run_command = f"{run_command} -w /workspace/tao_mm_workflows"
    ...
    mount_string = "-v {}:/workspace/tao_mm_workflows ".format(os.getenv("REPO_TOP", os.getcwd()))

    # Defining env variables.
    env_variables = "-e PYTHONPATH={}:$PYTHONPATH -e PYTHONDONTWRITEBYTECODE=1 ".format("/workspace/tao_mm_workflows")
    ...
    if port_mapping:
        print(f"Mapping ports: {port_mapping}")
        for port in port_mapping:
            if not ":" in port:
                print("Port mapping option is invalid.")
                continue
            port_option += f" -p {port}"
...
def parse_cli_args(args=None):
    ...
    parser = argparse.ArgumentParser(prog="tao_ws", description="Tool to run the container.", add_help=True)
...
def main(cl_args=None):
    ...
    args = parse_cli_args(tao_ws_args)
